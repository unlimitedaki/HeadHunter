...
class CSQAExample(MultipleChoiceExample):
    def __init__(
        ...
        super(CSQAExample, self).__init__(
            id, context, question, endings, label)
            ...
            for line in f.readlines():
                ...
                examples.append(
                    CSQAExample(
                        ...
                        context=None,
                        endings=[csqa_json["question"]["choices"][i]["text"]
                                 for i in range(5)],
                        label=label,
        ...
        for example_index, example in tqdm(enumerate(examples), total=len(examples), desc="CSQA processing"):
            ...
            for ending_index, ending in enumerate(example.endings):
                ...
                if example.context == None:
                    ...
                    sen1 = self.join_cs(
                        tokenizer, example.context[ending_index], example.question)
                    # sen1 = tokenizer.tokenize(sen1)
                    sen2 = tokenizer.tokenize(ending)
            ...
            for ending_index, ending in enumerate(example.endings):
                ...
                for cs in cs_list:
                    ...
                    input_tokens = question_tokens + \
                        [tokenizer.sep_token] + ending_tokens + \
                        [tokenizer.sep_token] + cs_tokens
...
def put_in_cs(examples, cs_data, omcs_corpus, cs_len):
    ...
    for example in tqdm(examples, total=len(examples), desc="puting commonsencs into examples"):
        ...
        for ending_index, ending in enumerate(example.endings):
            ...
            # extract first cs_len cs sentences for example
            cs_id_list = example_cs[ending_index]['cs'][:cs_len]
...
def feature_padding(args,
    ...
    if data_type == "dev":
        padding_input = torch.zeros(
            (padding_number, all_input_ids.shape[1], all_input_ids.shape[2]), dtype=torch.long)
        all_input_ids = torch.cat((all_input_ids, padding_input), dim=0)
        all_attention_masks = torch.cat(
            (all_attention_masks, padding_input), dim=0)
        all_token_type_ids = torch.cat(
            (all_token_type_ids, padding_input), dim=0)
        if all_labels != None:
            all_labels = torch.cat((all_labels, torch.tensor(
                [-1] * padding_number, dtype=torch.long)), dim=0)
    elif data_type == "train":
        padding_index = random.sample(range(0, cur_number), padding_number)
        all_input_ids = torch.cat((all_input_ids, torch.tensor(
            [all_input_ids[i].numpy() for i in padding_index], dtype=torch.long)), 0)
        all_attention_masks = torch.cat((all_attention_masks, torch.tensor(
            [all_attention_masks[i].numpy() for i in padding_index], dtype=torch.long)), 0)
        all_token_type_ids = torch.cat((all_token_type_ids, torch.tensor(
            [all_token_type_ids[i].numpy() for i in padding_index], dtype=torch.long)), 0)
        if all_labels != None:
            all_labels = torch.cat((all_labels, torch.tensor(
                [all_labels[i] for i in padding_index], dtype=torch.long)), 0)

    assert len(all_input_ids) == target_number
    print("{} set is padded to {} examples".format(
        data_type, str(target_number)))
...
class CSQALinearProcessor(CSQAProcessor):
    ...
    def convert_examples_to_features(self,
        ...
        def encode(example):
            ...
            attention_mask = []
            token_type_ids = []
            q_tokens = tokenizer(question, add_special_tokens=False,
                                 padding='max_length', truncation=True, max_length=question_seq_len-1)
            for ending_index, ending in enumerate(endings):
                cs_list = example.context[ending_index]
                # pdb.set_trace()
                try:
                    cs_tokens = tokenizer(cs_list, add_special_tokens=False,
                                          padding='max_length', truncation=True, max_length=cs_seq_len-1)
                    cs_joint_input_ids = []
                    cs_joint_attention_mask = []
                    # join cs encodings
                    for i, cs in enumerate(cs_tokens['input_ids']):
                        cs_joint_input_ids += [sep] + cs
                        cs_joint_attention_mask += [1] + \
                            cs_tokens['attention_mask'][i]
                except:
                    cs_joint_input_ids = [pad] * len(cs_list)*cs_seq_len
                    cs_joint_attention_mask = [0] * len(cs_list)*cs_seq_len
                a_tokens = tokenizer(ending, add_special_tokens=False,
                                     padding='max_length', truncation=True, max_length=answer_seq_len-1)

                # concat all tokens
                joint_input_ids = [cls] + q_tokens['input_ids'] + \
                    [sep] + a_tokens['input_ids'] + cs_joint_input_ids
                joint_attention_mask = [1] + q_tokens['attention_mask'] + \
                    [1] + a_tokens['attention_mask'] + cs_joint_attention_mask
                joint_token_type_ids = [1] * len(joint_attention_mask)
                joint_token_type_ids[:len(
                    q_tokens['input_ids'])+1] = [0] * (len(q_tokens['input_ids'])+1)
...
def load_csqa_omcs_dataset(tokenizer, args, omcs_corpus, data_type, is_training=True):
    ...
    cs_result_file = "{}_{}_omcs_of_dataset.json".format(
        data_type, args.cs_mode)
    ...
    if args.cs_len > 0:
        if data_type == "dev" or data_type == "test":
            examples = put_in_cs(
                examples, cs_data, omcs_corpus, args.dev_cs_len)
        else:
